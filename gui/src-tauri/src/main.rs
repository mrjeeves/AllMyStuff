...
#[tauri::command]
async fn local_file_locations(state: State<'_, AppState>) -> Result<Vec<LocalFileLocation>, String> {
...
#[tauri::command]
async fn fleet_storage_local_volumes(state: State<'_, AppState>) -> Result<Vec<FleetStorageVolume>, String> {
    let adapter_mounts = allmystuff_adapter_mounts(&state).await;
    Ok(allmystuff_inventory::scan()
        .storage
        .into_iter()
        .filter(|volume| volume.mount_point.as_deref().is_none_or(|path| {
            !adapter_mounts.iter().any(|mount| same_native_mount_path(path, mount))
        }))
